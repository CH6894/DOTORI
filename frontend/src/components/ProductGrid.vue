<!-- 예: src/components/BestSection.vue -->
  ...
  <section class="best">
    ...
    <div class="product-grid">
      <RouterLink
        ...
        :key="p.id"
        class="product-card"
        :to="{ name: 'product', params: { id: String(p.id) } }"
...
.product-card__thumb { aspect-ratio: 1 / 1; background: #f6f6f6; overflow: hidden; }
.product-card__thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
.product-card__meta { padding: 10px 12px 12px; display: flex; align-items: flex-start; flex-direction: column; gap: 4px; }
.product-card__name { font-size: 14px; line-height: 1.35; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.product-card__price { font-weight: 700; font-size: 14px; color: #333; }

.empty { grid-column: 1 / -1; text-align: center; padding: 36px 0; color: #9a9a9a; border: 1px dashed #e5e5e5; border-radius: 12px; }
