...
/** ───────────────────────── 장바구니 → 체크아웃 데이터 ─────────────────────────
 * CartView에서: /checkout?mode=selected&ids=1,2,3 또는 /checkout?mode=all
 * 여기서는 localStorage에서 전체를 읽고, 쿼리에 맞게 필터링
 */
...
const discount = computed(() =>
  items.value.reduce((s, it) => s + (Number((it as any).discount) || 0), 0)
)
const subtotal = computed(() =>
  items.value.reduce((sum, it) => sum + it.price * it.qty, 0)
)
const shippingFee = computed(() =>
  items.value.reduce((sum, it) => sum + (it.shipping || 0), 0)
)
...
const onPay = () => {
  ...
  // 필요 시 여기서 결제 요청 → 성공 시 라우터로 완료 페이지 이동하도록 변경 가능
}
</script>
...
.checkout {
  max-width: 720px;
  margin: 24px auto 120px; /* 하단 고정바 공간 확보 */
  padding: 0 16px;
}
...
.card {
  ...
  /* 가운데 넓게 쓰고 싶다면 아래 두 줄 유지, 아니면 삭제 */
  width: 140%;
  position: relative;
  left: 50%;
...
.addr-label {
  width: 70px; font-weight: 600; color: #555;
  flex-shrink: 0; text-align: left; margin-right: 12px;
}
...
.btn-primary.note {
  width: 100%; margin-top: 12px; background: #ff7a2e; color: #fff; border: none;
  border-radius: 10px; padding: 12px 20px; font-weight: 700;
  display: flex; align-items: center; justify-content: space-between;
}
.note .chev {
  width: 18px; height: 18px; stroke: #fff; stroke-width: 2.5; fill: none;
  display: block; line-height: 0; transform-box: fill-box; transform-origin: 40% 50%;
  transition: transform .2s ease;
}
.note.open .chev { transform: rotate(180deg); }

.note-panel {
  margin-top: 10px; background: #fff; border: 1px solid #eee;
  border-radius: 10px; padding: 12px;
}
.note-textarea {
  width: 100%; resize: vertical; border: 1px solid #e6e6e6; border-radius: 8px;
...
.btn-subtle {
  background: #f6f6f6; border: 1px solid #e6e6e6; border-radius: 8px; padding: 8px 12px; cursor: pointer;
}
.btn-save {
  background: #ff7a2e; color: #fff; border: none; border-radius: 8px; padding: 8px 14px; font-weight: 700; cursor: pointer;
}
...
.thumb img {
  width: 72px; height: 72px; object-fit: cover; border-radius: 8px; border: 1px solid #eee;
}
...
.bank-box {
  margin-top: 10px; background: #fff; border: 1px solid #eee; border-radius: 10px; padding: 12px;
...
.select, .input {
  width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; box-sizing: border-box; background: #fff;
}
.select {
...
.notice {
  background: #fff7f1; border: 1px solid #ffd7bf; color: #000;
  padding: 10px 12px; border-radius: 8px; line-height: 1.5; font-size: 14px;
}
