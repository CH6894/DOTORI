...
public class InspectionService {
    ...
    public Page<AdminListRow> getFilteredAdminList(Integer state, LocalDateTime from, LocalDateTime to,
    ...
    private Integer parseRejectionReason(String reason) {
        ...
        if (reason.contains("촬영 각도/장면 부족"))
            return 1;
        if (reason.contains("해상도/초점 문제"))
            return 2;
        if (reason.contains("라벨/시리얼 확인 불가"))
            return 3;
        if (reason.contains("상품 상태 설명 불충분"))
            return 4;
        if (reason.contains("광고/워터마크 포함"))
            return 5;
    ...
    public Admin getInspectionDecision(String inspectionId) {
        ...
        System.out.println("- ItemExplanation: " + admin.getItemExplanation());
