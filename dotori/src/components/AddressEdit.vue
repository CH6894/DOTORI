        ...
        <div class="modal">
          ...
          <div class="modal-body">
            ...
            <!-- <div class="field inline">
              <label class="grow">
                <span>우편번호</span>
                <input v-model="form.postcode" type="text" placeholder="00000" />
              </label>
              <button class="btn-outline sm" @click="onSearchPostcode">우편번호</button>
            </div> -->

            <label class="field">
              <span>주소</span>
              <input v-model="form.addr1" type="text" placeholder="기본 주소" />
            </label>
...
export default {
  name: 'AddressEdit', // 다단어 컴포넌트명
  props: {
    modelValue: { type: Boolean, default: false }, // v-model 제어
    value: {
      type: Object,
      default: () => ({
        receiver: '',
  ...
  emits: ['update:modelValue', 'save', 'search-postcode'],
  setup(props, { emit }) {
    // 폼 상태 (모달 열릴 때마다 props.value로 리셋)
    const form = reactive({
      receiver: '',
      phone: '',
      postcode: '',
    ...
    const onSave = () => {
      if (!form.receiver?.trim() || !form.phone?.trim() || !form.addr1?.trim()) {
    ...
    const onSearchPostcode = () => {
      emit('search-postcode') // 부모에서 다음/카카오 우편번호 팝업 열기
    }
...
/* .field.inline { grid-template-columns: 1fr auto; align-items: end; gap: 8px; } */
